...
function filterByTitle(recentMovies, searchedTitle) {
  return recentMovies.filter((recentMovies) =>
    recentMovies.title.toLowerCase().includes(searchedTitle.toLowerCase())
  );
}

const res = filterByTitle(recentMovies, "coucou");
const res1 = filterByTitle(recentMovies, "the");
